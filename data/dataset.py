"""
data/dataset.py
3DFaceARC Dataset — handles unannotated single-view face images.
...
class MediaPipeLandmarkExtractor:
    ...
    def __init__(self):
        ...
        self.face_mesh = None   # lazy: created on first use
    ...
    def extract(self, image_rgb: np.ndarray):
        ...
        return {'lmks_68': lmks_68, 'lmks_468': pts}
...
class FaceDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        if self.cache_landmarks and key in self._landmark_cache:
            cache = self._landmark_cache[key]
            crop_box = cache['crop_box']
            landmarks = cache['landmarks']
        else:
            ext_results = self.landmark_extractor.extract(img_rgb)
            h, w = img_rgb.shape[:2]
            
            if ext_results is None:
                # No face detected: Centre crop fallback
                m = 0.10
                crop_box = (int(w*m), int(h*m), int(w*(1-m)), int(h*(1-m)))
                landmarks = np.zeros((68, 2), dtype=np.float32)
            else:
                pts = ext_results['lmks_468']
                lmks_68 = ext_results['lmks_68']
                ...
                c_x1 = max(0, int(x_min - margin_x))
                c_y1 = max(0, int(y_min - margin_y))
                c_x2 = min(w, int(x_max + margin_x))
                c_y2 = min(h, int(y_max + margin_y))
                crop_box = (c_x1, c_y1, c_x2, c_y2)
                
                # Adjust landmarks to crop space
                lmks_68[:, 0] -= c_x1
                lmks_68[:, 1] -= c_y1
                
                # Scale landmarks to target image_size
                crop_w = c_x2 - c_x1
                crop_h = c_y2 - c_y1
                if crop_w > 0 and crop_h > 0:
                    lmks_68[:, 0] = lmks_68[:, 0] / crop_w * self.image_size
                    lmks_68[:, 1] = lmks_68[:, 1] / crop_h * self.image_size
                
                # Normalize to [-1, 1]
                landmarks = lmks_68 / (self.image_size / 2.0) - 1.0

            if self.cache_landmarks:
                self._landmark_cache[key] = {
                    'crop_box': crop_box,
                    'landmarks': landmarks
                }
        ...
        face_crop = img_rgb[c_y1:c_y2, c_x1:c_x2]
        if face_crop.size == 0:
            face_crop = img_rgb # Fallback
