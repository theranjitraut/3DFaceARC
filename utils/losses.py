"""
utils/losses.py
Self-supervised loss functions for 3DFaceARC.
...
class VGGPerceptualLoss(nn.Module):
    ...
    NOTE: Always runs in float32 regardless of AMP context.
    VGG16's BatchNorm layers overflow in fp16 → NaN loss.
    """
    ...
    def forward(self, rendered: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        Runs in float32 to prevent fp16 overflow.
        """
        # Force float32 — VGG BN overflows in fp16, producing NaN that
        # propagates through the entire loss and poisons GradScaler.
        with torch.autocast(device_type='cuda', enabled=False):
            r = self._normalise(rendered.float().clamp(0, 1))
            t = self._normalise(target.float().clamp(0, 1))
            loss = torch.tensor(0.0, device=rendered.device)
            for sl in self.slices:
                r_feat = sl(r)
                t_feat = sl(t)
                loss = loss + F.l1_loss(r_feat, t_feat)
...
def landmark_loss(pred_verts: torch.Tensor, faces: torch.Tensor,
    ...
    pred_lmks_norm = pred_lmks / (max_val + 1e-6)
...
def coeff_regularisation(coeffs: dict, weights: dict = None) -> torch.Tensor:
    ...
    for key, w in weights.items():
        if key in coeffs:
            loss = loss + w * (coeffs[key] ** 2).mean()
    return loss


# Mesh Smoothness (Laplacian) Loss  — with cached indices
class LaplacianSmoothnessLoss(nn.Module):
    """
    Uniform Laplacian smoothness loss with cached topology indices.

    The face topology (idx_i, idx_j, deg) is the same every batch since
    the BFM mesh structure never changes.  Computing them from 94K faces
    each forward pass wastes ~0.5-1 s/batch.  This module caches them
    after the first call, keyed by (n_faces, device).
    """

    def __init__(self):
        super().__init__()
        self._cache: dict = {}   # key: (n_faces, device_str) → (idx_i, idx_j, deg)

    def _get_laplacian_indices(self, faces: torch.Tensor):
        key = (int(faces.shape[0]), str(faces.device))
        if key not in self._cache:
            a, b, c = faces[:, 0], faces[:, 1], faces[:, 2]
            idx_i = torch.cat([a, b, a, c, b, c])
            idx_j = torch.cat([b, a, c, a, c, b])
            V = int(faces.max().item()) + 1
            deg = torch.zeros(V, device=faces.device)
            deg.scatter_add_(0, idx_i, torch.ones(idx_i.shape[0], device=faces.device))
            deg = deg.clamp(min=1)
            self._cache[key] = (idx_i, idx_j, deg)
    ...
    def forward(self, verts: torch.Tensor, faces: torch.Tensor) -> torch.Tensor:
        ...
        faces: (F, 3)
        """
        idx_i, idx_j, deg = self._get_laplacian_indices(faces)
        B = verts.shape[0]
        ...
        for b in range(B):
            ...
            nbr_sum.scatter_add_(
                0, idx_j.unsqueeze(1).expand(-1, 3), v[idx_i]
            )
            laplacian = v - nbr_sum / deg.unsqueeze(1)      # (V, 3)
            total = total + (laplacian ** 2).mean()
        return total / B
...
def _safe(loss_val: torch.Tensor, name: str) -> torch.Tensor:
    """Return zero instead of NaN/Inf so one bad component can't poison total."""
    if not torch.isfinite(loss_val):
        # Must require grad so PyTorch doesn't crash on .backward()
        z = torch.zeros_like(loss_val)
...
class 3DFaceARCLoss(nn.Module):
    ...
    def __init__(self, cfg: dict):
        ...
        self.laplacian_loss  = LaplacianSmoothnessLoss()
    ...
    def forward(self, model_out: dict, batch: dict) -> dict:
        ...
        # 1. Photometric
        losses['photometric'] = _safe(
            photometric_loss(rendered, target, silhouette) * self.w['photometric'],
            'photometric'
        )

        # 2. Perceptual  (always float32 inside VGGPerceptualLoss)
        losses['perceptual'] = _safe(
            self.perceptual_net(rendered, target) * self.w['perceptual'],
            'perceptual'
        )

        # 3. Landmark (pseudo)
        losses['landmark'] = _safe(
            landmark_loss(verts, faces, lmks) * self.w['landmark'],
            'landmark'
        )

        # 4. 3DMM regularisation
        losses['shape_reg'] = _safe(
            coeff_regularisation(coeffs) * self.w['shape_reg'],
            'shape_reg'
        )

        # 5. Mesh smoothness (cached Laplacian)
        losses['smooth'] = _safe(
            self.laplacian_loss(verts, faces) * self.w['smooth'],
            'smooth'
        )

        # 6. Symmetry
        losses['symmetry'] = _safe(
            symmetry_loss(rendered) * self.w['symmetry'],
            'symmetry'
        )

        # 7. Identity
        losses['id_preserve'] = _safe(
            identity_loss(
