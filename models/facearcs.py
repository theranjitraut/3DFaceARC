...
class 3DFaceARC(nn.Module):
    ...
    def __init__(self, cfg: dict):
        ...
        # 3. Determine intermediate feature-map channels BEFORE building sampler
        # ResNet50 layer3 outputs 1024 channels; EfficientNet uses 0 (no hook)
        self._feat_map = None
        if hasattr(self.encoder.backbone, 'layer3'):
            self.feat_map_channels = 1024   # ResNet layer3 actual channels
        else:
            self.feat_map_channels = 256    # EfficientNet fallback

        # 4. Vertex feature sampler
        ...
        self.gcn_refiner = BatchedGCNRefiner(
            ...
            max_verts=gcn_cfg.get('max_verts', 8192)
        )
        ...
        # 7. ArcFace (frozen, for ID loss)
        self.arcface = ArcFaceEncoder(
            weights_path=cfg['paths'].get('pretrained_arcface', '')
        )

        # 8. Register forward hook to capture the feature map at runtime
    ...
    def forward(self, x: torch.Tensor) -> dict:
        ...
        if self._feat_map is not None:
            ...
            # Only sample up to configured max_verts (default 6000) for memory efficiency
            sample_v = min(self.n_verts, self.cfg['model']['gcn'].get('max_verts', 6000))
        ...
        # log.info("    [DEBUG-FWD] Step 6: ArcFace")
