...
def _make_synthetic_bfm(n_shape=80, n_exp=64, n_tex=80, n_verts=5023):
    """
...
# BFM Loader
def load_bfm(model_path: str, n_shape=80, n_exp=64, n_tex=80):
    """
    Load the Basel Face Model from .mat file.
    Falls back to synthetic model if file not found.
    """
    ...
    try:
        ...
        bfm = {
            ...
            'faces':       raw['tri'].astype(np.int64) - 1,  # 1-indexed → 0-indexed
            'n_verts':     raw['meanshape'].shape[0] // 3,
        }
        print(f"[BFM] Loaded real BFM: {bfm['n_verts']} vertices, "
